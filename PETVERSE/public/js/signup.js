...
function validateAdditionalFields() {
    ...
    if (userType === 'pet-seller') {
        ...
        if (!businessName.value.trim()) {
            ...
            businessName.closest('.input-group').appendChild(createErrorMessage('✖ Business name is required'));
            businessName.closest('.input-group').classList.add('has-error');
            isValid = false;
        }
        
        if (!licenseNumber.value.trim()) {
            clearErrors(licenseNumber);
            licenseNumber.closest('.input-group').appendChild(createErrorMessage('✖ License number is required'));
            licenseNumber.closest('.input-group').classList.add('has-error');
            isValid = false;
        }
    }
    ...
    if (userType === 'service-provider') {
        ...
        if (!serviceType.value) {
            ...
            serviceType.closest('.input-group').appendChild(createErrorMessage('✖ Service type is required'));
            serviceType.closest('.input-group').classList.add('has-error');
            isValid = false;
        }
        
        if (!certification.value.trim()) {
            clearErrors(certification);
            certification.closest('.input-group').appendChild(createErrorMessage('✖ Certification is required'));
            certification.closest('.input-group').classList.add('has-error');
            isValid = false;
        }
    }
...
loginForm.addEventListener('submit', function (e) {
    ...
    const isAdditionalFieldsValid = validateAdditionalFields();
    ...
    if (isEmailValid && isUsernameValid && isPasswordValid && isConfirmPasswordValid && 
        isPhoneValid && isAdditionalFieldsValid && isAgreeChecked) {
